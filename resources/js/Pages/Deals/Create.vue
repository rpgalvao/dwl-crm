...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Cadastrar Nova Negociação</h2>
        </template>
            ...
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 border border-gray-200">
                    ...
                    <form @submit.prevent="submit" class="space-y-8">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Título da Negociação</label>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Cliente (Contato)</label>
                            <select
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="R$ 0,00"
                                >
                                <div v-if="form.errors.estimated_value" class="text-red-500 text-sm mt-1">{{ form.errors.estimated_value }}</div>
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Previsão de Fechamento</label>
                                <input
                                    v-model="form.expected_closed_at"
                                    type="date"
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 text-gray-800"
                                >
                                <div v-if="form.errors.expected_closed_at" class="text-red-500 text-sm mt-1">{{ form.errors.expected_closed_at }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end space-x-4 border-t border-gray-100 pt-6 mt-6">
                            ...
                            <button
                                ...
                                class="bg-blue-600 text-white px-8 py-3 rounded-md hover:bg-blue-700 transition font-bold disabled:opacity-50 shadow-sm"
                            >
                                Salvar Negociação
                            </button>
                        </div>
